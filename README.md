...
- 📋 **A plan waiting for approval reads out too.** "Ready to code?" is the same kind of block as a question, so it gets the same purple tab and the same click-to-talk — the title and the size from across the room, the plan's whole outline when you click in.
...
**A plan waiting for approval is a question too.** "Ready to code? Here is Claude's plan" stops the session exactly the way an `AskUserQuestion` does — it's your move, and until you make it nothing happens — so it gets the whole purple treatment: the tab colors while it's in the background, clicking in reads it out, answering shuts it up mid-sentence, and `hold` silences the unprompted half of that and not the click-in. What's different is length. A question is a sentence; a plan is pages. So the unfocused cue gives you the **title and the size** — *"probot: a plan is waiting on your call. Finish 2026.3.1, build QA adjudication, scope the permanent Probot. 8 steps."* — and **clicking in reads the plan's shape**: the title, then every section heading, *"8 steps: Context; the provenance finding; Thread 1, generalize customer-flagging; …"*. Capping the prose was the obvious alternative and it's worse — a long plan's cap lands mid-context, so you'd hear two minutes of preamble and never reach the steps you're being asked to approve. (A plan written as flat prose has no headings, and there the capped body *is* the outline.) **`/tts plan full`** reads the whole plan, uncapped, for approving without going to look; **`/tts plan off`** leaves plans alone entirely.
...
| `/tts plan <gist\|full\|off>` | A plan awaiting approval: read its title and section headings on click-in (default), read the whole plan, or ignore plans |
