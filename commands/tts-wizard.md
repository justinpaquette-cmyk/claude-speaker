...
2. **Give the tour**, compactly — a short table, not paragraphs. Mark each line
   ...
   | **Click-to-talk** (`focus_speak`) | Being at a waiting terminal reads it out — clicking in, or a summary arriving in the tab you're already watching. Any age: a colored tab is one you haven't heard. Clicking into an open *question* reads the question too |
   | **Backlog cap** (`recap_max`) | A click-in or a bare `rr all` speaks the newest 3, oldest-first so it ends on the newest, and counts the rest ("8 older updates skipped") rather than reading a whole session back. Skipped ones still get marked played; `rr 10` / `rr 30m` replay them, uncapped |
   ...
   | **Question cue** (`ask_color`, `ask_speak`) | 🟣 an AskUserQuestion is open — the one color that means *you* are the blocker. Background tabs only (the tab you're in already shows the question), outranks the waiting ladder, never ages. `ask_speak` reads the question's headers aloud when it opens unfocused — silent under `hold`, which means nothing speaks at a terminal you aren't watching. **Clicking in** reads the question plus its option labels, and that one is *not* silenced by `hold` — clicking in is you asking. Multi-question asks are followed one at a time (`ask_follow`): `screen` (default) watches the tab's visible text while you sit on it and reads each question as you reach it, cutting over when you advance; `click` reads the next unheard question per click-in; `off` is first-question-only. `ask_reads` (default 1) caps re-reads of an already-heard set. Answering, switching away, or moving past a question stops its readout. Moving *past* one un-hears it so arrowing back re-reads it; switching away from the tab does not, because leaving usually means you went to fetch the answer. `ask_context` (default on) opens the first read with the words Claude wrote before asking — the why, capped like a summary. `ask_first` (default off) reads question one the moment a set opens in the focused tab — no replay exists for a missed question, unlike `rr` for summaries |
   | **Plan cue** (`plan`) | A plan waiting for approval ("Ready to code?") is the same state as an open question — session stopped, next move yours — so it gets the same 🟣 tab, click-to-talk and gates. Opening unfocused it speaks the title and the size ("a plan is waiting on your call … 8 steps"); clicking in reads the plan's *shape*, title plus every section heading, because a cap on the prose lands mid-context and never reaches the steps. `full` reads the whole plan uncapped, for approving without looking; `off` leaves plans alone entirely |
